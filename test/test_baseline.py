...
class TestBaseline(unittest.TestCase):
    def setUp(self):
        ...
        self.stock = CsvReader(data_dir).read_stock(["SPY", "GOOG", "GLD"], [Column.Name.ADJCLOSE])

    def test_spy(self):
        baseline_stock = Baseline("SPY").process(self.stock)
        self.assertEqual(baseline_stock.column(Column.Name.ADJCLOSE).df.shape[0], 50)

    def test_goog(self):
        baseline_stock = Baseline("GOOG").process(self.stock)
        self.assertEqual(baseline_stock.column(Column.Name.ADJCLOSE).df.shape[0], 30)
